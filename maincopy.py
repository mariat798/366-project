    ...
    def assigned_cells(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                ...
                if domain == 1:
                    assignments.append((i, j, int(grid.get_cells()[i][j])))
...
class Backtracking:
    ...
    def search(self, grid, var_selector, ac3, Q, bad_assignments = []):
        ...
        current_assignments = ac3.assigned_cells(grid)

        for bad_assignment in bad_assignments:
            all_exist = True
            
            for coord_value in bad_assignment:
                if not coord_value in current_assignments:
                    all_exist = False
                    break
        ...
        bad_assignments.append(ac3.assigned_cells(grid))
        return None
